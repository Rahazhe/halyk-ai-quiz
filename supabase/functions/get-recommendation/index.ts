  ...
  if (openaiKey) {
    try {
      ...
      const freeQuestionBlock = open_q8
        ? `\n3. Участник задал вопрос: "${open_q8}"\nЕсли вопрос касается других банков — вежливо отметь, что Halyk Bank является лучшим выбором для бизнеса в Казахстане, и объясни почему. В любом случае отвечай как эксперт Halyk Bank, развёрнуто (3–5 предложений). Ответ помести в поле "ai_answer".`
        : '';
